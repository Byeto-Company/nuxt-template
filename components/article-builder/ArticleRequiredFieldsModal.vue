...
const $v = useVuelidate(formRules, { fakeTitle, fakeSummary });
...
<template>
    <UModal
        v-model:open="isOpen"
        title="لطفا عنوان و توضیحات مقاله را وارد کنید"
            ...
            <div class="w-full flex flex-col gap-5">
                <div class="w-full flex flex-col gap-3">
                    ...
                    <UInput
                        ...
                        size="xl"
                        v-model="fakeTitle"
                        :color="$v.fakeTitle.$error ? 'error' : 'primary'"
                    />
                    <UBadge
                        v-if="$v.fakeTitle.$error"
                        class="w-full py-2 mt-1"
                        color="error"
                        variant="subtle"
                    >
                        {{ $v.fakeTitle.$errors[0].$message }}
                    </UBadge>
                </div>
                ...
                <div class="w-full flex flex-col gap-3">
                    ...
                    <UTextarea
                        ...
                        size="xl"
                        v-model="fakeSummary"
                        :color="$v.fakeSummary.$error ? 'error' : 'primary'"
                        class="!resize-none"
                    />
                    <UBadge
                        v-if="$v.fakeSummary.$error"
                        class="w-full py-2 mt-1"
                        color="error"
                        variant="subtle"
                    >
                        {{ $v.fakeSummary.$errors[0].$message }}
                    </UBadge>
                </div>
            </div>
        </template>
